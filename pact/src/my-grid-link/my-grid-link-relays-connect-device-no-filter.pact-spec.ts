...
describe('Pact consumer test', () => {
  ...
  describe('myGrid.link trigger relays connect device with device ids', () => {
    beforeAll(done => {
      provider
        .addInteraction({
          state: 'A_REQUEST_MY_GRID_LINK_FOR_TRIGGER_RELAYS_CONNECT_DEVICE_WITH_IDS',
          uponReceiving: 'a request for trigger relays connect device with ids in request - myGrid.Link',
          withRequest: {
            method: service.postRelaysConnectDeviceRequest(requestBody).method,
            path: service.postRelaysConnectDeviceRequest(requestBody).url,
            body: requestBody,
    ...
    it('should make request for trigger relays Connect device with ids in request - myGrid.Link', done => {
      service.postRelaysConnectDevice(requestBody).subscribe(
