...
describe('Pact consumer test', () => {
  ...
  describe('myGrid.link delete device with filter request', () => {
    beforeAll((done) => {
      provider
        .addInteraction({
          state: 'A_REQUEST_MY_GRID_LINK_FOR_DELETE_DEVICE_WITH_FILTER',
          uponReceiving: 'a request for delete device with filter in request - myGrid.Link',
          withRequest: {
            method: 'DELETE', // service.deleteDeviceRequest(requestBody).method,
            path: service.deleteDeviceRequest(requestBody).url,
            body: requestBody,
    ...
    it('should make request for delete device with filter in request - myGrid.Link', (done) => {
