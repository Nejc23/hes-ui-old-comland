...
describe('Pact consumer test', () => {
  ...
  describe('myGrid.link trigger relays disconnect device with device ids', () => {
    beforeAll(done => {
      provider
        .addInteraction({
          state: 'A_REQUEST_MY_GRID_LINK_FOR_TRIGGER_RELAYS_DISCONNECT_DEVICE_WITH_IDS',
          uponReceiving: 'a request for trigger relays disconnect device with ids in request - myGrid.Link',
          withRequest: {
            method: service.postRelaysDisconnectDeviceRequest(requestBody).method,
            path: service.postRelaysDisconnectDeviceRequest(requestBody).url,
            body: requestBody,
    ...
    it('should make request for trigger relays disconnect device with ids in request - myGrid.Link', done => {
